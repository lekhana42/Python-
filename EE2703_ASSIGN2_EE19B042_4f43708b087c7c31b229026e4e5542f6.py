...
class one_port_element():
   #General class for all the elements 
    def __init__(self,line):
        ...
        if len(self.tokens) == 5:       #dc case
            self.type = 'dc'
            self.value = float(self.tokens[4])
        elif len(self.tokens) == 6:      #ac case
            self.type = 'ac'
...
try:
    if( len(sys.argv) != 2):
        sys.exit('Invalid number of arguments') #verifying input
    name = sys.argv[1]
    f = open(name)
    lines = f.readlines()
    f.close()
    ...
    for i in range(len(diction)):       #matrix
    ...
    try:
        x = np.linalg.solve(M,b)   #solving mtrix 
    except Exception:
